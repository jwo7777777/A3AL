...
switch (playerSide) do
{
	case west:
	{
		...
		__CONST__(life_adminlevel_877344,parseNumber(_session select 7));
		life_blacklisted = call compile format["%1",(_session select 8)];
		[] spawn life_fnc_loadGear;
		//if(life_adminlevel_877344 > 0) then {[] execVM "core\client\aconfig.sqf";};
	...
	case civilian:
	{
		...
		__CONST__(life_adminlevel_877344,parseNumber(_session select 6));
